 ...
 */
var ChargeOrder = {
    id: "ChargeOrderChannelTable",	//表格id
    seItem: null,		//选中的条目
    table: null,
    layerIndex: -1
};
...
ChargeOrder.loadMoney = function(){
    var ajax = new $ax(Feng.ctxPath + "/chargeOrder/allMoneyChannel", function (data) {
        $("#allMoney").val(data);
        // ChargeOrder.table.refresh();
    }, function (data) {
...
ChargeOrder.loadPerson = function(){
    var ajax = new $ax(Feng.ctxPath + "/chargeOrder/allPersonChannel", function (data) {
        $("#allPerson").val(data);
        // ChargeOrder.table.refresh();
    }, function (data) {
        Feng.error("失败!" + data.responseJSON.message + "!");
...
$(function () {
    ...
    var table = new BSTable(ChargeOrder.id, "/chargeOrder/channelList", defaultColunms);
