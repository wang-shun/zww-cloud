...
DivinationImageInfoDlg.addSubmit = function() {
    ...
    // }
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/divinationImage/add?temp=1", function(data){
        Feng.success("添加成功!");
 ...
 * 加载默认值
 */
DivinationImageInfoDlg.init = function(column,value){
 ...
 * 修改初始化表单
 */
DivinationImageInfoDlg.editInit = function() {

    DivinationImageInfoDlg.init("divinationTopicId");
    if($("#divinationTopicIdValue").val() == undefined){
        $("#divinationTopicId").attr("value","");
    }else{
        $("#divinationTopicId").attr("value",$("#divinationTopicIdValue").val());
    }
...
$(function() {
    ...
    DivinationImageInfoDlg.editInit();
    // 初始化头像上传
    var avatarUp = new $WebUpload("divinationTopicImg");
    avatarUp.setUploadBarId("progressBar");
    avatarUp.setUploadUrl(Feng.ctxPath + '/divinationImage/uploads/'+DivinationImageInfoDlg.get('divinationTopicId'));
    avatarUp.init();
});
